 ...
 * Module Dependencies
 */
var debug = require('debug')('loopback-passport:AuthModel');
...
module.exports = function(AuthModel) {
  ...
  AuthModel.login = function(ctx, Provider, credentials, include, fn) {
    ...
    if (thisProviderOptions.isLocal) {
      ...
      passport.authenticate(Provider, loginSettings)(ctx.req, ctx.res);
    }
  };
  /**
  ...
  AuthModel.loginCallback = function(ctx,Provider,fn){
    var thisProvider = passport._strategies[Provider],
        thisProviderOptions = thisProvider.ProviderOptions;
    if (!thisProvider || HiddenStaregies.indexOf(Provider) >= 0) {
      // ProviderNotFound
      var ProviderNotFound = new Error('Provider not found.');
    ...
    var callbackTranslator = passportCallbackTranslator(fn,afterVerify);
    passport.authenticate(Provider,loginSettings,callbackTranslator)(ctx.req,ctx.res);
  };
  ...
  AuthModel.getCurrentUser = function(ctx,fn){
    var AccessTokenModel = AuthModel.app.models.AccessToken;
    AccessTokenModel.findForRequest(ctx.req, {}, function (err, token) {
      if (err){ return fn(err);}
      if (!token){return fn(new Error('Not logged in.'));} // No need to throw an error here

      // Logic borrowed from user.js -> User.logout() to get access token object
      var UserModel = AuthModel.app.models.User;
      UserModel.relations.accessTokens.modelTo.findById(token.id, function(err, accessToken) {
        if (err){ return fn(err);}
        if (!accessToken){ return fn(new Error('could not find the given token'));}

        // Look up the user associated with the access token
        UserModel.findById(accessToken.userId, function (err, user) {
          if (err){ return fn(err);}
          if (!user){ return fn(new Error('could not find a valid user with the given token'));}
          return fn(null,user);
          // Here if user is found
          // 
        });
  ...
  AuthModel.logout = function(ctx,fn) {};

  AuthModel.use = function() {
    passport.use.apply(passport, arguments);
  ...
  AuthModel.configureProvider = function(ProviderName, ProviderOptions) {
    ...
    passport._strategies[ProviderName].ProviderOptions = ProviderOptions;
  ...
  AuthModel.setup = function() {
    ...
    resultAuthProvider.remoteMethod('getCurrentUser',{
      ...
      returns: {
        ...
        type: 'object',
        root: true,
        description: 'The data of the current logged in user.'
      },
      http: [{
        verb: 'get',
        path: '/currentUser'
